using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using System.Threading.Tasks;

namespace DataLayer.Entities.Portal
{
    public class Tbl_Pazirande
    {
        [Key]
        public int Id { get; set; }

        public string Status { get; set; }

        public string Description { get; set; }

        public string UserName { get; set; }

        public string Type_Moshtari { get; set; }

        public string Code_Meli { get; set; }

        public string Shenase_Meli { get; set; }

        public string Shomare_Shenasname { get; set; }

        public string Name { get; set; }

        public string Family { get; set; }

        public DateTime Tarikh_Tavalod { get; set; }

        public string Name_Pedar { get; set; }

        public string Jensiat { get; set; }

        public string Mobile { get; set; }
        public string Address { get; set; }

        public string Name_EN { get; set; }

        public string Family_EN { get; set; }

        public string Pedar_EN { get; set; }

        public string Name_Sherkat { get; set; }

        public DateTime Tarikh_Tasis_sherkat { get; set; }

        public string Name_EN_Sherkat { get; set; }

        public string Code_Egtesadi { get; set; }

        public string Name_Tejari { get; set; }

        public string Shomare_Sabt { get; set; }

        public bool Pazirande_Atba { get; set; }

        public string Code_Faragir { get; set; }

        public string Code_Gozarname { get; set; }

        public DateTime Tarikh_Etbar { get; set; }

        public string Melyat { get; set; }

        public string Shenase_Onvan_Nemayandegi { get; set; }

        public string Shenase_Bazaryab_Nemayandegi { get; set; }

        public string Shenase_Porojeh { get; set; }

        public string Shenase_Switch { get; set; }

        public string Shenase_Type_Dargah { get; set; }

        public string Shenase_Type_Dastgah { get; set; }

        public string Shenase_Model_Dastgah { get; set; }

        public string Shenase_Bank { get; set; }

        public string Code_Shobe { get; set; }

        public string Shomare_Hesab { get; set; }

        public string Shomare_Sheba { get; set; }

        public string Code_Shaparaki_shahr { get; set; }

        public string Mantage_Shahri { get; set; }

        public string Code_Posti { get; set; }

        public string TelePhone { get; set; }

        public string Fax { get; set; }

        public string Code_Takmily_Senf { get; set; }

        public string Onvan_Mahal_Kasb { get; set; }

        public string Onvan_Latin_Kasb { get; set; }
<<<<<<< HEAD
=======

>>>>>>> 934adf13578939b19c3ca9f693f05e4422b673e2
        public string Shenase_Malekyat { get; set; }
        public string Address_Shaparaki { get; set; }

        public string Address_Latin { get; set; }

        public string Shenase_Mahal_kasb { get; set; }

        public string Shomare_Javaz_kasb { get; set; }

        public string Marja_Saderkonande_Mojavez { get; set; }

        public DateTime Tarikh_Sodor_Javaz { get; set; }

        public DateTime Tarikh_Payan_Etebar { get; set; }

        public string Shomare_Garardad_Ejare { get; set; }

        public DateTime Tarikh_Etmam_Ejare { get; set; }

        public string Code_Rahgiri { get; set; }

        public string Type_Garardad_Hesab { get; set; }

        public string Shenase_Type_Vagozari { get; set; }

        public string Type_Vadieh { get; set; }

        public string Shomare_Sanad { get; set; }

        public string Mablag_Frosh { get; set; }

        public string Moaref { get; set; }

        public string Phone_Moaref { get; set; }

        public string Tedad_Agsat { get; set; }

        public string Mablag_Agsat { get; set; }

        public string Mobail_Motasel { get; set; }

        public string Name_Fani_Pos { get; set; }

        public string Mobaile_Fani_Pos { get; set; }

        public string Telephone_Fani_Pos { get; set; }

        public string Shomare_Pazirande { get; set; }

        public string Shomare_Terminal { get; set; }

        public string Serial_Dastgah { get; set; }

        public DateTime Tarikh_Nasb { get; set; }

        public string Shomare_Marja { get; set; }

        public DateTime Tarikh_Tarif_Switch { get; set; }

        public DateTime Tarikh_Tarif_Shaparak { get; set; }

        public string Nemad_Elektroniki { get; set; }

        public DateTime Tarikh_Etmam_Etebar { get; set; }

        public DateTime Tarikh_Sodor_Nemad { get; set; }

        public string Type_Nemad { get; set; }

        public string Server_Frosh { get; set; }

        public string IPGAccessPort { get; set; }

        public string IPGCallBackAddress { get; set; }

        public string Address_Post_Sherkat { get; set; }

        public string Mahsolat_Frosh { get; set; }

        public string Address_Post { get; set; }

        public string Family_Masol_Fani { get; set; }

        public string Mobail_Masol_Fani { get; set; }

        public string Name_Fani_Masol_Fani { get; set; }

        public string Code_Meli_Masol_Fani { get; set; }

        public string Address_Site { get; set; }

        public string Emkan_Lagv { get; set; }

        public string Zamine_Faliat { get; set; }
    }
}
